...
bool SoundPlayer::SetUpALSourceIdle() const {
	...
	alSourcef(audio_source.source, AL_PITCH, parameters.pitch);
	...
	if (parameters.local) {
		...
		* our direction would have more gain than if it's firing while facing back to us */
		...
		alSource3f(audio_source.source, AL_DIRECTION, u, w, v);
#endif
		alSource3f(audio_source.source, AL_POSITION, positionX, positionZ, positionY);
		SetUpALSource3D();
	}
...
bool SoundPlayer::SetUpALSourceInit() const {
	...
	if (parameters.local) {
		...
		alSourcei(audio_source.source, AL_DISTANCE_MODEL, AL_LINEAR_DISTANCE_CLAMPED);
		alSourcei(audio_source.source, AL_SOURCE_RELATIVE, AL_FALSE);
	}
...
void SoundPlayer::SetUpALSource3D() const {
	...
	if (parameters.behavior == _sound_is_quiet && obstruction) {
		alSourcef(audio_source.source, AL_GAIN, 0);
		return;
	}

	//One more rule for this case
	if (parameters.behavior == _sound_is_loud && !obstruction) {
		alSourcef(audio_source.source, AL_MIN_GAIN, calculated_volume / 8);
	}
	...
	alSourcef(audio_source.source, AL_REFERENCE_DISTANCE, behaviorParameters.distance_reference);
	alSourcef(audio_source.source, AL_MAX_DISTANCE, behaviorParameters.distance_max);
	alSourcef(audio_source.source, AL_ROLLOFF_FACTOR, behaviorParameters.rolloff_factor);
	alSourcef(audio_source.source, AL_MAX_GAIN, behaviorParameters.max_gain * calculated_volume);
	alSourcef(audio_source.source, AL_GAIN, behaviorParameters.max_gain * calculated_volume);
}
