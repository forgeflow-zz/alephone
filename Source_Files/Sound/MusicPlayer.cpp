...
bool MusicPlayer::SetUpALSourceIdle() const {
	alSourcef(audio_source.source, AL_MAX_GAIN, default_volume * OpenALManager::Get()->GetComputedVolume(filterable));
	alSourcef(audio_source.source, AL_GAIN, volume ? volume * OpenALManager::Get()->GetComputedVolume(filterable) : default_volume * OpenALManager::Get()->GetComputedVolume(filterable));
	return true;
}